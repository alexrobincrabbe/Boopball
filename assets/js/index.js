...
function throwBall() {
  ...
  if (isAiming && !isThrowing) {
    ...
    yVel =
      (force * Math.sin((-rotation * 2 * Math.PI) / 360) * timeStep) / 1000;
    isAiming = false;
    isThrowing = true;
    setMoveball = setInterval(() => moveBall(timeStep), timeStep);
  }
...
function moveBall(timeStep) {
  ...
  if (
    xPosBall > xPosHoop - xVel - 30 &&
    xPosBall < xPosHoop + hoopSize - xVel + 30 &&
    yPosBall < yPosHoop - yVel &&
    yPosBall >= yPosHoop &&
    yVel < 0 &&
    scoreReady == true
  ) {
    ...
    if (
      xPosBall < xPosHoop - xVel + 25 ||
      xPosBall > xPosHoop + hoopSize - xVel - 25
    ) {
      yVel = -yVel / 2;
    } else if (stageReady) {
      score += 1;
  ...
  if (
    xPosBall > xPosHoop &&
    xPosBall < xPosHoop + hoopSize &&
    yPosBall + ballSize > yPosHoop - yVel &&
    yPosBall <= yPosHoop &&
    yVel > 0 &&
    !bumped
  ) {
